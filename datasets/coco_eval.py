        ...
        for iou_type in iou_types:
            ...
            if iou_type == "keypoints":
                if self.num_kpts == 21:
                    ...
                    print("[CocoEvaluator] num_kpts=21, 使用 hand-21 OKS sigmas:", self.sigmas)
                    coco_eval.params.kpt_oks_sigmas = self.sigmas
                    print(f"params:{coco_eval.params.kpt_oks_sigmas}")
                else:
                    # 保留官方默认（比如 COCO body-17）
                    self.sigmas = np.array(coco_eval.params.kpt_oks_sigmas, dtype=np.float32)
