        ...
        if 'keypoints' in tgt:
            ...
            for idx, ann in enumerate(tgt['keypoints']):
                if "kpt_bbox" in tgt:
                    ...
                    ax.add_collection(p_kpt)
                kp = np.array(ann.cpu())
                assert kp.ndim == 1, f"tgt['keypoints'] 期望是一维扁平 (3K,), 实际是 {kp.shape}"
                num_kpts = kp.shape[0] // 3 
                Z = kp[:2 * num_kpts] * np.array([W, H] * num_kpts)  # (2K,)
                V = kp[2 * num_kpts:]                         
                x = Z[0::2]
                y = Z[1::2]
                v = V
                ...
                # 扩展，以免颜色越界
                if len(color_kpt) < num_kpts:
                    extra = num_kpts - len(color_kpt)
                    for _ in range(extra):
                        color_kpt.append(
                            (np.random.random((1, 3)) * 0.6 + 0.4).tolist()[0]
                        )
